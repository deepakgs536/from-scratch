...
const fetchProductDetails = async (items: any[]) => {
  ...
  const allProductIds = [...new Set(items.map((i: any) => i.productId))];
...
export const OrderDetails = () => {
    ...
    const fetchOrder = async () => {
      try {
        if (id) {
          ...
          const detailsMap = await fetchProductDetails(orderData.items);
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      await PaymentAPI.update(paymentId, { status: 'PAID' });
        ...
        {isCancelled ? (
          ...
          <div className="relative pt-4">
            <div className="absolute left-6 top-4 bottom-4 w-1 bg-slate-100 md:w-auto md:h-1 md:top-10 md:left-6 md:right-6 md:bottom-auto z-0 -translate-x-1/2 md:translate-x-0 md:-translate-y-1/2 rounded-full">
               <div className="bg-primary absolute left-0 top-0 transition-all duration-1000 ease-in-out rounded-full" style={{ width: window.innerWidth >= 768 ? `${Math.max(0, currentStageIndex) * 33.33}%` : '100%', height: window.innerWidth < 768 ? `${Math.max(0, currentStageIndex) * 33.33}%` : '100%' }} />
            </div>
          ...
          <form onSubmit={handleUpdate} className="space-y-8">
            <div className="grid md:grid-cols-2 gap-6">
              <div className="space-y-2 md:col-span-2">
                ...
                <Input id="street" placeholder="123 Main St" value={editForm.street} onChange={(e) => setEditForm({...editForm, street: e.target.value})} required className="bg-slate-50/50 border-slate-200 h-12 rounded-xl" />
              </div>
              <div className="space-y-2">
                <label htmlFor="city" className="text-sm font-bold text-slate-700">City</label>
                <Input id="city" placeholder="City" value={editForm.city} onChange={(e) => setEditForm({...editForm, city: e.target.value})} required className="bg-slate-50/50 border-slate-200 h-12 rounded-xl" />
              </div>
              <div className="space-y-2">
                <label htmlFor="state" className="text-sm font-bold text-slate-700">State / Province</label>
                <Input id="state" placeholder="State" value={editForm.state} onChange={(e) => setEditForm({...editForm, state: e.target.value})} required className="bg-slate-50/50 border-slate-200 h-12 rounded-xl" />
              </div>
              <div className="space-y-2">
                <label htmlFor="zip" className="text-sm font-bold text-slate-700">Zip / Postal Code</label>
                <Input id="zip" placeholder="Zip" value={editForm.zip} onChange={(e) => setEditForm({...editForm, zip: e.target.value})} required className="bg-slate-50/50 border-slate-200 h-12 rounded-xl" />
              </div>
              <div className="space-y-2">
                <label htmlFor="contact" className="text-sm font-bold text-slate-700">Contact Number</label>
                <Input id="contact" placeholder="+1 (555) 000-0000" value={editForm.contact_number} onChange={(e) => setEditForm({...editForm, contact_number: e.target.value})} className="bg-slate-50/50 border-slate-200 h-12 rounded-xl" />
              </div>
              <div className="space-y-2 md:col-span-2">
                <label htmlFor="instructions" className="text-sm font-bold text-slate-700">Delivery Instructions (Optional)</label>
                <Input id="instructions" placeholder="e.g., Leave at front door" value={editForm.delivery_instructions} onChange={(e) => setEditForm({...editForm, delivery_instructions: e.target.value})} className="bg-slate-50/50 border-slate-200 h-12 rounded-xl" />
              </div>
            </div>
         ...
         <div className="p-6 space-y-6">
           ...
           {['PENDING'].includes(order.status) && (
             <div className="bg-blue-50 p-8 rounded-2xl border border-blue-100 text-center shadow-sm relative overflow-hidden">
               ...
               <Button size="lg" className="px-10 h-12 rounded-xl text-base font-bold shadow-md shadow-primary/20 hover:-translate-y-1 transition-all duration-300 w-full md:w-auto" disabled={isUpdating} onClick={handlePayment}>
  ...
  return (
    <div className="container mx-auto py-12 px-6 max-w-5xl">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6 bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
        <div>
          <h1 className="text-2xl font-bold tracking-tight mb-1 text-slate-900">{orderTitle}</h1>
          <p className="text-slate-500 text-sm flex items-center gap-2 font-medium">
            <Clock className="w-4 h-4" /> Placed on {order.created_at ? format(new Date(order.created_at), 'MMMM dd, yyyy - hh:mm a') : 'Unknown'}
          </p>
        </div>
      ...
      <div className="flex overflow-x-auto space-x-2 mb-10 pb-2 no-scrollbar border-b border-slate-200">
        {tabs.map((tab) => {
          ...
          return (
            <button
              type="button" key={tab.id} onClick={() => setActiveTab(tab.id as any)}
              className={`flex items-center whitespace-nowrap px-6 py-3 text-sm font-bold transition-colors relative rounded-full ${isActive ? 'text-primary' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-100'}`}
            >
              <tab.icon className="w-4 h-4 mr-2" /> {tab.label}
              {isActive && <motion.div layoutId="activeTabIndicator" className="absolute bottom-[-9px] left-0 right-0 h-1 bg-primary rounded-t-full" initial={false} transition={{ type: "spring", stiffness: 500, damping: 30 }} />}
            </button>
          );
        })}
      </div>
