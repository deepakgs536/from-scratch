...
const loadProductsWithImages = async (productsData: any[]) => {
  return await Promise.all(
    productsData.map(async (product: any) => {
      ...
      try {
        ...
        return { ...product, image_url: mediaResponse.data.url };
      } catch (err) {
        console.error(`Failed to generate signed URL for ${product.productId}`, err);
...
const mergeProductsWithInventory = (products: any[], inventory: any[]) => {
  return products.map((product: any) => {
    const inv = inventory.find((item: any) => item.productId === product.productId) || {
...
export const AdminInventory = () => {
  ...
  const fetchInventory = async () => {
    ...
    try {
      ...
      const products = await loadProductsWithImages(productsRes.data.data);
  ...
  const renderLowStockList = () => {
    const lowStockItems = inventoryItems.filter(i => i.available_quantity <= 10);
    return (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-amber-800">
              Low Stock & Depleted Items ({lowStockItems.length})
            </span>
          </div>
          {showLowStock ? (
